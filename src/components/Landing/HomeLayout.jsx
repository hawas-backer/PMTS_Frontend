...
const HomeLayout = () => {
  ...
  return (
    <div className="min-h-screen text-[var(--primary-text)] bg-[var(--primary-bg)]">
      <a
        href="#main-content"
        className="fixed top-0 left-0 p-2 bg-[var(--primary-accent)] text-white focus:top-4 focus:left-4 z-50 transition-all duration-300"
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
          <motion.div className="flex items-center space-x-3" whileHover={{ scale: 1.05 }}>
            <img src={logo} alt="GCEK Logo" className="h-10 w-10 object-contain rounded-full border-2 border-[var(--primary-accent)] p-1 bg-white shadow-md" />
            <h1 className="text-lg font-semibold hidden md:block">Placement Cell</h1>
          </motion.div>
            ...
            {navItems.map((item) => (
              <motion.div key={item.label} className="relative group" whileHover={{ scale: 1.05 }}>
                <Link
                  ...
                  offset={-100}
                  duration={800}
                  className="font-medium text-[var(--primary-text)] opacity-80 hover:opacity-100 transition duration-300"
                  onSetActive={() => setActiveSection(item.href)}
                >
                  <item.icon className="w-4 h-4 mr-1.5 inline-block text-[var(--primary-accent)]" />
                  {item.label}
                </Link>
                <motion.div
          ...
          <button
            className="md:hidden text-[var(--primary-accent)] focus:outline-none z-50"
        ...
        {isMenuOpen && (
          <motion.div
            className="md:hidden bg-[var(--secondary-bg)] shadow-md"
            ...
            <div className="p-4 space-y-3">
              {navItems.map((item) => (
                <Link
                  ...
                  offset={-100}
                  duration={800}
                  className="block text-[var(--secondary-text)] font-medium hover:text-[var(--primary-accent)] transition-colors duration-300 py-2"
                  onSetActive={() => {
